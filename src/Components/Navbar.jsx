    ...
    <div className="bg-white shrink-0 h-20 w-full fixed top-0   z-50">
      <div className=" pt-5 flex items-center ml-4">
        {toggle ? (
          <Eye
            onClick={() => handleMenu()}
            className="h-8 w-8 text-blue-800 xl:hidden transition"
          />
        ) : (
          <EyeClosed
            onClick={() => handleMenu()}
            className="h-8 w-8 text-red-800 xl:hidden transition"
          />
        )}

        <div
          className="bg-slate-200  h-14 w-60 ml-2 flex justify-center
